import { Range, Event, EventEmitter, ExtensionContext, SymbolKind, SymbolInformation, TextDocument, TextEditor, TreeDataProvider, TreeItem, TreeItemCollapsibleState, commands, window, workspace, Uri, Position, Location } from 'vscode';
    ...
    private async updateSymbols(editor:TextEditor): Promise<void> {
        if (editor) {
            ...
            let result = pythonShell.run('script.py', options,  (err, results) => {
                if (!err) {
                    this.nodes = new Array<SymbolNode>();
